#!/usr/bin/env bash
# AI Code Assistant v25.1 - The Chunking Hotfix
...
# --- Principles ---
# 1. Intelligent Chunking: Automatically breaks massive file batches into smaller, manageable chunks to avoid system limits.
# 2. Resilient Processing: Works seamlessly with the queue-reloop logic to retry failed chunks, not the entire job.
...
MODEL="$DEFAULT_MODEL"
MAX_RELOOPS=3
CHUNK_SIZE=500 # Max number of files to process in a single API call
CONTEXT_STRING=""
...
self_update() {
    ...
    echo "🖋️ Opening editor to update script. Paste new code, save, and exit."
    sleep 1; nano "$TEMP_FILE"
    if [[ -s "$TEMP_FILE" ]]; then mv -f "$TEMP_FILE" "$SCRIPT_PATH"; chmod +x "$SCRIPT_PATH"; echo "🎉 Script updated successfully."; else echo "❌ Update aborted as no content was saved."; rm -f "$TEMP_FILE"; fi
    exit 0
}

auto_select_best_model() {
    if [[ "$MODEL" != "$DEFAULT_MODEL" ]]; then echo "ℹ️ Using user-specified model: '$MODEL'"; return; fi
    echo "ℹ️ Auto-selecting the best available Gemma model..."; local preferred_models=("gemma3:1b" "gemma2:latest" "gemma:2b"); local installed_models; installed_models=$(ollama list | awk 'NR>1 {print $1}')
    for preferred_model in "${preferred_models[@]}"; do if echo "$installed_models" | grep -q -x "$preferred_model"; then MODEL="$preferred_model"; echo "✅ Found and selected model: '$MODEL'"; return; fi; done
    echo "⚠️ Default model '$DEFAULT_MODEL' not found locally. Using fallback 'gemma2:latest'. For optimal speed, please run 'ollama pull $DEFAULT_MODEL'"
    MODEL="gemma2:latest"
}

load_file_instructions() {
    if [[ -f "./.ai_instructions" ]]; then
        echo "ℹ️ Found '.ai_instructions', loading file-specific context."
...
ensure_ollama_is_running() {
    if ! curl -s --fail "$OLLAMA_API_URL" -o /dev/null; then
        echo "🔌 Ollama server not responding. Starting a new instance..."; ollama serve & disown
        for i in {1..10}; do if curl -s --fail "$OLLAMA_API_URL" -o /dev/null; then echo "✅ Ollama server is now online."; return 0; fi; sleep 1; done
...
scan_for_media() {
    echo "🖼️ Scanning for media assets in the current directory..."
...
generate_code_from_prompt() {
    ...
    echo -e "🧠 \e[90mOllama is thinking...\e[0m" >&2
...
process_single_file_instance() {
    ...
    echo "🔍 Showing diff for '$DISPLAY_NAME'..."; local source_file_for_diff="$([ -e "$REAL_PATH" ] && echo "$REAL_PATH" || echo "/dev/null")"
    ...
    if [[ "$CONFIRM" =~ ^[Yy]$ ]]; then
        if [[ -s "$REAL_PATH" ]]; then local BACKUP_PATH="$BACKUP_DIR/$(basename "$REAL_PATH")_$(date '+%Y%m%d%H%M%S').bak"; cp "$REAL_PATH" "$BACKUP_PATH"; log_action "UPDATE: '$REAL_PATH' (Backup: '$BACKUP_PATH')"; echo "✅ '$DISPLAY_NAME' updated."; else log_action "CREATE: '$REAL_PATH'"; echo "✅ '$DISPLAY_NAME' created."; fi
        mv "$TMP_FILE" "$REAL_PATH"; return 0
    else
        rm "$TMP_FILE"; log_action "DISCARD: '$DISPLAY_NAME'"; echo "❌ Changes for '$DISPLAY_NAME' discarded. (AI output is archived)."; return 1
    fi
}
...
run_single_file_mode() {
    ...
    echo "🚀 Engaging Single-File Mode (Language: $LANGUAGE) for: $DISPLAY_NAME"; local generated_code; generated_code=$(generate_code_from_prompt "$final_prompt"); process_single_file_instance "$REAL_PATH" "$DISPLAY_NAME" "$generated_code"
}
...
run_batch_mode() {
    ...
    if [[ "$total_files" -le "$CHUNK_SIZE" ]]; then
        ...
        echo "📦 Batch is too large ($total_files files). Processing in $num_chunks chunks of up to $CHUNK_SIZE..."
...
run_batch_mode_pass() {
    ...
    while [[ ${#files_queue_map[@]} -gt 0 && $reloop_count -lt $MAX_RELOOPS ]]; do
        reloop_count=$((reloop_count+1)); local current_queue_keys=("${!files_queue_map[@]}"); echo "🔄 Processing Pass ${reloop_count}/${MAX_RELOOPS}. Files in queue: ${#current_queue_keys[@]}"
...
EOF
        ...
        if ! echo "$CLEAN_JSON" | jq empty 2>/dev/null; then echo "⚠️ Invalid JSON in pass ${reloop_count}, retrying... The AI returned:" >&2; echo "---" >&2; echo "$CLEAN_JSON" >&2; echo "---" >&2; log_action "WARN: Invalid JSON in pass ${reloop_count}. AI returned: $CLEAN_JSON"; sleep 1; continue; fi
        ...
        while IFS= read -r line; do
            ...
            local DISPLAY_NAME="${FILE_MAP[$REAL_PATH]}"; local NEW_B64; NEW_B64=$(echo "$line" | jq -r '.content_base_64')
...
main() {
    ...
    for action_string in "${action_queue[@]}"; do
        ...
        for ARG in "${FILES_ARGS[@]}"; do
            ...
            [[ ! "$ARG" =~ [*?\[] && ! -e "$ARG" ]] && echo "✨ File '$ARG' does not exist. Creating..." && touch "$ARG"
    ...
    echo "🎉 All tasks complete. See logs at '$LOG_FILE' and archives at '$ARCHIVE_DIR'."
}
